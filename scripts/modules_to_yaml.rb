...
framework.modules.each do |_, m|
  ...
  next if date.empty?
  modules << {}
  modules[-1]['description'] = m.description.gsub(/\n/, ' ').gsub(/\s+/, ' ').strip
  modules[-1]['name'] = m.fullname
  modules[-1]['published'] = Time.parse(date).utc
  modules[-1]['updated'] = Time.parse(`git log --format=%ai "#{m.file_path}" | head -n 1`.strip).utc
  modules[-1]['vulnerabilities'] = []
  m.references.each { |reference| modules[-1]['vulnerabilities'] << "#{reference.ctx_id}-#{reference.ctx_val}" if reference.ctx_id == "CVE" }
end
